            ...
            <div className='max-w-6xl px-2 md:px-5 flex flex-wrap justify-center items-center gap-3 md:gap-6'>
              {
                data.map((value, index) => (
                  <Link href={`/Category/Productshow/${value._id}`} key={value._id + '-' + value.quantity} className={`w-43 md:w-55 text-black border-x-1 border-b-1 border-black/10 relative hover:scale-105 transform transition-transform duration-300  rounded-md md:p-3 pb-4 bg-white`}>
                    ...
                    <h1 className='font-sans font-semibold text-[13px] md:text-[15px] pt-3 max-md:px-1.5'>{value.name}</h1>
                        ...
                        value.stock === 'stock' ?
                          buttonspin ?
                            loadingProductId === value._id ?
                              <button className='text-white transition-transform active:scale-95 duration-100 relative group bg-teal-600 mx-2 flex justify-center items-center z-999 rounded-sm w-full h-7 text-[14px] mt-3 tracking-wider' style={{ fontFamily: 'serif' }}>
                              ...
                              <button onClick={(e) => {
                                ...
                                e.stopPropagation();
                                addtocart(value._id)
                              }} className='text-white transition-transform active:scale-95 duration-100 relative group bg-teal-500 z-999 mx-2 rounded-sm w-full h-7 text-[14px] mt-3 tracking-wider' style={{ fontFamily: 'serif' }}>
